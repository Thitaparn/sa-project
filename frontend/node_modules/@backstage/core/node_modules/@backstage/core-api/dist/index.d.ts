...
import React$1, { FC, ReactNode, ComponentType, ComponentClass, FunctionComponent } from 'react';
...
declare type ApiFactory<Api, Deps extends {
...
declare type AnyApiFactory = ApiFactory<unknown, {
    [key in string]: unknown;
}>;
declare type ApiFactoryHolder = {
    get<T>(api: ApiRef<T>): ApiFactory<T, {
        [key in string]: unknown;
    }> | undefined;
};
...
declare function createApiFactory<Api>(api: ApiRef<Api>, instance: Api): ApiFactory<Api, {}>;
...
declare const UserIcon: React$1.ComponentType<OverrideProps<SvgIconTypeMap<{}, "svg">, "svg">>;
declare const GroupIcon: React$1.ComponentType<OverrideProps<SvgIconTypeMap<{}, "svg">, "svg">>;

declare const resolveRoute: unique symbol;
declare const routeReference: unique symbol;
declare type ReferencedRoute = {
    [routeReference]: unknown;
};
declare type ConcreteRoute = ReferencedRoute & {
...
declare type SubRouteConfig = {
    path: string;
};
declare class SubRouteRef<T extends {
    [name in string]: string;
} | never = never> implements ReferencedRoute {
    private readonly parent;
    private readonly config;
    constructor(parent: ConcreteRoute, config: SubRouteConfig);
    get [routeReference](): this;
    link<Args extends T extends never ? [] : [T]>(...args: Args): ConcreteRoute;
}
declare class AbsoluteRouteRef implements ConcreteRoute {
    private readonly config;
    ...
    get path(): string;
    get title(): string;
    createSubRoute<T extends {
        [name in string]: string;
    } | never = never>(config: SubRouteConfig): SubRouteRef<T>;
    get [routeReference](): this;
    [resolveRoute](path: string): string;
}
declare function createRouteRef(config: RouteRefConfig): AbsoluteRouteRef;
declare type MutableRouteRef = AbsoluteRouteRef;
...
declare class GithubAuth implements OAuthApi, SessionApi {
    ...
    static create({ discoveryApi, environment, provider, oauthRequestApi, }: CreateOptions): GithubAuth;
...
declare class GitlabAuth {
    static create({ discoveryApi, environment, provider, oauthRequestApi, }: CreateOptions$1): typeof gitlabAuthApiRef.T;
}
...
declare class GoogleAuth {
    static create({ discoveryApi, oauthRequestApi, environment, provider, }: CreateOptions$2): typeof googleAuthApiRef.T;
}
...
declare class OAuth2 implements OAuthApi, OpenIdConnectApi, ProfileInfoApi, BackstageIdentityApi, SessionApi {
    static create({ discoveryApi, environment, provider, oauthRequestApi, defaultScopes, scopeTransform, }: CreateOptions$3): OAuth2;
    private readonly sessionManager;
...
declare class OktaAuth {
    static create({ discoveryApi, environment, provider, oauthRequestApi, }: CreateOptions$4): typeof oktaAuthApiRef.T;
}
...
declare class SamlAuth implements ProfileInfoApi, BackstageIdentityApi, SessionApi {
    private readonly sessionManager;
    static create({ discoveryApi, environment, provider, }: CreateOptions$5): SamlAuth;
    sessionState$(): Observable<SessionState>;
...
declare class Auth0Auth {
    static create({ discoveryApi, environment, provider, oauthRequestApi, }: CreateOptions$6): typeof auth0AuthApiRef.T;
}

declare type CreateOptions$7 = {
    discoveryApi: DiscoveryApi;
    oauthRequestApi: OAuthRequestApi;
    environment?: string;
    provider?: AuthProvider & {
...
declare class MicrosoftAuth {
    static create({ environment, provider, oauthRequestApi, discoveryApi, }: CreateOptions$7): typeof microsoftAuthApiRef.T;
}
...
export { AbsoluteRouteRef, AlertApi, AlertApiForwarder, AlertMessage, AnyApiFactory, AnyApiRef, ApiFactory, ApiFactoryHolder, ApiHolder, ApiProvider, ApiRef, ApiRefConfig, ApiRefType, ApiRefsToTypes, ApiRegistry, AppComponents, AppConfigLoader, AppOptions, AppTheme, AppThemeApi, AppThemeSelector, Auth0Auth, AuthProvider, AuthRequestOptions, AuthRequester, AuthRequesterOptions, BackstageApp, BackstageIdentity, BackstageIdentityApi, BackstagePlugin, BootErrorPageProps, ConcreteRoute, ConfigApi, CreateStorageApiOptions, DiscoveryApi, ErrorAlerter, ErrorApi, ErrorApiForwarder, ErrorContext, FeatureFlagName, FeatureFlagOutput, FeatureFlagState, FeatureFlags, FeatureFlagsApi, FeatureFlagsRegistryItem, GithubAuth, GithubSession, GitlabAuth, GoogleAuth, GroupIcon, IconComponent, IdentityApi, LegacyRedirectRouteOutput, LegacyRouteOutput, MicrosoftAuth, MutableRouteRef, OAuth2, OAuth2Session, OAuthApi, OAuthRequestApi, OAuthRequestManager, OAuthScope, Observable, Observer, OktaAuth, OpenIdConnectApi, PendingAuthRequest, PluginOutput, ProfileInfo, ProfileInfoApi, RedirectRouteOutput, RouteOptions, RouteOutput, RoutePath, RouteRef, RouteRefConfig, SamlAuth, SessionApi, SessionState, SignInPageProps, SignInResult, StorageApi, StorageValueChange, Subscription, SystemIconKey, SystemIcons, TypesToApiRefs, UrlPatternDiscovery, UserIcon, WebStorage, alertApiRef, appThemeApiRef, auth0AuthApiRef, configApiRef, createApiFactory, createApiRef, createPlugin, createRouteRef, defaultSystemIcons, discoveryApiRef, errorApiRef, featureFlagsApiRef, githubAuthApiRef, gitlabAuthApiRef, googleAuthApiRef, identityApiRef, microsoftAuthApiRef, oauth2ApiRef, oauthRequestApiRef, oktaAuthApiRef, samlAuthApiRef, storageApiRef, useApi, useApiHolder, useApp };
