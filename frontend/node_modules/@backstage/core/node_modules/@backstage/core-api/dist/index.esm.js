...
const resolveRoute = Symbol("resolve-route");
const routeReference = Symbol("route-ref");
...
class SubRouteRef {
  ...
  link(...args) {
    return {
      ...
      [resolveRoute]: (path) => {
        var _a;
        const ownPart = generatePath(this.config.path, (_a = args[0]) != null ? _a : {});
...
class AbsoluteRouteRef {
  ...
  createSubRoute(config) {
    return new SubRouteRef(this, config);
  }
  get [routeReference]() {
    return this;
  }
...
class PrivateAppImpl {
  ...
  getApiHolder() {
    ...
    for (const plugin2 of this.plugins) {
      for (const factory of plugin2.getApis()) {
        if (!registry.register("default", factory)) {
          throw new Error(`Plugin ${plugin2.getId()} tried to register duplicate or forbidden API factory for ${factory.api}`);
        }
      }
    }
