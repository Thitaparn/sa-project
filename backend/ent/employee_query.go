...
func (eq *EmployeeQuery) QueryPatients() *PatientQuery {
...
func (eq *EmployeeQuery) WithPatients(opts ...func(*PatientQuery)) *EmployeeQuery {
	query := &PatientQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withPatients = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withPatients; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.PatientsColumn, fks...))
		}))
