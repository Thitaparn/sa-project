...
func (eu *EmployeeUpdate) AddPatients(p ...*Patient) *EmployeeUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (eu *EmployeeUpdate) RemovePatients(p ...*Patient) *EmployeeUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (eu *EmployeeUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := eu.mutation.RemovedPatientsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   employee.PatientsTable,
			Columns: []string{employee.PatientsColumn},
			Bidi:    false,
	...
	if nodes := eu.mutation.PatientsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   employee.PatientsTable,
...
// AddPatients adds the patients edges to Patient.
func (euo *EmployeeUpdateOne) AddPatients(p ...*Patient) *EmployeeUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// RemovePatients removes patients edges to Patient.
func (euo *EmployeeUpdateOne) RemovePatients(p ...*Patient) *EmployeeUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (euo *EmployeeUpdateOne) sqlSave(ctx context.Context) (e *Employee, err error) {
	...
	if nodes := euo.mutation.RemovedPatientsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   employee.PatientsTable,
	...
	if nodes := euo.mutation.PatientsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   employee.PatientsTable,
